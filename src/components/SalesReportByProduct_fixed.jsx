...
function SalesReportByProduct({ storeId }) {
  ...
  const loadSalesReport = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_BASE_API_URL}/orders/store/${storeId}?${queryParams}`, {
        headers: {
      ...
      if (response.ok) {
        ...
        if (data.success && data.data && data.data.orders) {
          ...
          const processedReport = processOrdersForSalesReport(data.data.orders, storeId);
